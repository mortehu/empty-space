...
void GL::initialize()
{
  ...
  void* library = System::dlopen(glDriver.string);
...
#define PROC_EXT(x) \
  ...
  strcpy(config.renderer,
         reinterpret_cast<const char*>(getString(GL::RENDERER)));
  strcpy(config.vendor,
         reinterpret_cast<const char*>(getString(GL::VENDOR)));
  strcpy(config.version,
         reinterpret_cast<const char*>(getString(GL::VERSION)));
  strcpy(config.extensionString,
         reinterpret_cast<const char*>(getString(GL::EXTENSIONS)));
